...
if getconnections then
    ...
    game:GetService("StarterGui"):SetCore("SendNotification",{
        Title = "Success!",
        Text = "Added optional WalkSpeed bypass.",
        Icon = "rbxassetid://1"
    })
    wait()
...
if hookmetamethod then
    ...
    wait()
    print("[LOG OUT]: Temporarily disabled due to unexpected issues with hooking WalkSpeed, JumpPower, etc")
    --[[wait()
...
if setmetatable and getmetatable and cloneref then
    ...
    --[[CharacterTab:AddButton({
        ...
        Callback = function()
        if getconnections and Human and Humanoid.Health >= 99 and HumanoidRP ~= nil then
            ...
            Library:MakeNotification({
                Name = "Success!",
                Content = "WalkSpeed bypass executed.",
                Image = "rbxassetid://4483345998",
                Time = 10
            })
        elseif Human == nil or Human.Health <= 1 or HumanoidRP == nil then
            repeat wait() until Human and HumanoidRP
            if Human and HumanoidRP then
                pcall(function()
                    for i,v in pairs(getconnections(game:GetService("Players").LocalPlayer.Character:WaitForChild("Humanoid", 0.5):GetPropertyChangedSignal("WalkSpeed"))) do
                        v:Disable()
                    end
                end)
            end
        elseif not getconnections then
            return Library:MakeNotification({
                Name = "Error:",
                Content = "You cant run this! (missing getconnections)",
                Image = "rbxassetid://4483345998",
                Time = 10
            })
        end
    ...
    PrisonLifePage:AddButton({
        ...
        Callback = function()
            print(".")
            local Gun = game.Players.LocalPlayer.Backpack:FindFirstChild("Remington 870") or game.Players.LocalPlayer.Character:FindFirstChild("Remington 870")
            local Space = workspace.Prison_ITEMS.giver:FindFirstChild("Remington 870"):FindFirstChild("ITEMPICKUP")
            if not Gun then
                game:GetService("Players").LocalPlayer.Character:PivotTo(Space:GetPivot())
                wait(0.5)
                workspace.Remote.ItemHandler:InvokeServer(Space)
            else
            ...
            local Space1 = workspace.Prison_ITEMS.giver:FindFirstChild("AK-47"):FindFirstChild("ITEMPICKUP")
            if not Gun then
                game:GetService("Players").LocalPlayer.Character:PivotTo(Space1:GetPivot())
                wait(0.5)
                workspace.Remote.ItemHandler:InvokeServer(Space1)
            else
                print("Already has weapon")
            end
            print("h 1")
            local Gun = game.Players.LocalPlayer.Backpack:FindFirstChild("M9") or game.Players.LocalPlayer.Character:FindFirstChild("M9")
            local Space2 = workspace.Prison_ITEMS.giver:FindFirstChild("M9"):FindFirstChild("ITEMPICKUP")
            if not Gun then
                game:GetService("Players").LocalPlayer.Character:PivotTo(Space2:GetPivot())
                wait(0.5)
                workspace.Remote.ItemHandler:InvokeServer(Space2)
            else
                print("Already has weapon")
            end
            print("h 2")
    ...
    CharacterTab:AddSlider({
        ...
        if Human.Health >= 99 and HumanoidRP and HumanoidRP ~= nil then
            ...
            repeat wait() until Human and Human ~= nil and HumanoidRP and HumanoidRP ~= nil
            if Human and HumanoidRP then
                Human.WalkSpeed = speedFunc
            end
        end
...
-- Does not work without necessary functions in exploit AKA, 'getmetatable' and 'setmetatable' which exploits without it obviously do not have, e.g, Solara, FluxTeam, etc, (Usually level 3 executors, will be broken if uncommented, and should only be used on powerful executors.)
...
-- This function below is broken and I am still working on pushing an update for it, but for now i'm leaving this here, feel free to release a fix for it, if you want.
